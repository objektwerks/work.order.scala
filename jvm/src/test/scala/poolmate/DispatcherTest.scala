...
class DispatcherTest extends AnyFunSuite with Matchers with LazyLogging:
  ...
  def testDispatcher(dispatcher: Dispatcher, store: Store): Unit = {
    ...
    var pool = Pool(license = account.license, name = "test", built = 20010101, cost = 25000.00)
    ...
    var timer = Timer(poolId = pool.id, installed = 20010101, model = "intermatic", cost = 75.00)
    ...
    var heater = Heater(poolId = pool.id, installed = 20010201, model = "hayward", cost = 2500.00)
